...
export const BOOKS: Book[] = [
  ...
  },
  {
    ...
    contentForPrompt: `너무너무 지루한 지룽이. 에밀은 침대에 앉아 있고 방에는 책도 많고 장난감도 많지만, 아무것도 하지 않아 지루하다. 똑똑 소리에 창밖을 보니 손이 없고 몸이 길쭉하며 키가 아주 큰 누군가가 뾰족한 코로 창문을 두드리고 있다. 에밀이 창문을 열자 연기처럼 스며들어 들어온 존재는 자신을 "너무너무 지루한 지룽이"라고 소개하며 아무것도 하지 않는다고 말한다. 에밀은 함께 놀고 싶어 책을 읽어 달라거나 이야기를 해 달라고 하지만 지룽이는 글씨를 못 읽고 아는 이야기도 없다고 한다. 에밀은 '아무렇게나 지어내면 된다'며 장난감 악어 프리츠와 인형 펠린느를 내밀어 이야기를 만들기 시작한다. 처음에는 지룽이가 말도 안 된다며 심통을 부리지만, 에밀은 보물 상자(조개껍데기, 새 깃털, 젤리 곰 등)를 보여 주고 장난감 강아지 둘과 호랑이를 '보물을 지키는 무서운 용'으로 바꾸며 모험 이야기를 이어 간다. 베개와 방석으로 성을 쌓고, 용을 달래 과자와 케이크를 주는 장면까지 만들면서 에밀은 점점 신나게 몰입한다. 그 사이 지룽이는 침대 밑에서 투덜대다가 에밀이 "나와서 같이 놀자!"라고 하자 슬그머니 나오는데, 아까보다 훨씬 작아지고 짧아져 있다. 지룽이는 "여긴 내가 있을 데가 아니야. 사실 나는 처음부터 여기 없었어."라고 말하며 연기처럼 스르르 창밖으로 빠져나간다. 에밀은 지룽이를 잠깐 내려다본 뒤 다시 성과 보물, 용과 호랑이에 정신이 팔려 앞으로의 모험을 기대한다.`,
  },
  {
    id: 'book-willy',
    title: '윌리가 보는 세상',
